        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {PAGES.map((p) => (
            <button
              ...
              className={`text-left bg-white rounded-xl border p-4 hover:shadow-md transition-all ${expandedPage === p.name ? "border-indigo-300 ring-2 ring-indigo-100" : "border-slate-200/60"}`}
            >
              ...
              <p className="text-[10px] text-indigo-600 font-medium mt-2">Route: <code className="bg-slate-100 px-1 py-0.5 rounded text-[10px]">{p.route}</code></p>
            </button>
          ))}
        </div>
      </div>
            ...
            <div className="space-y-3">
              {section.fields.map((f, i) => (
                <div key={i}>
                  ...
                  {f.type === "textarea" ? (
                    <textarea defaultValue={f.value} rows={2} className="w-full px-3 py-2 text-sm border border-slate-200 rounded-lg bg-slate-50 focus:bg-white focus:ring-2 focus:ring-indigo-500/30 focus:outline-none resize-none" />
                  ) : (
                    <input defaultValue={f.value} className="w-full px-3 py-2 text-sm border border-slate-200 rounded-lg bg-slate-50 focus:bg-white focus:ring-2 focus:ring-indigo-500/30 focus:outline-none" />
                  )}
                </div>
              ))}
            </div>
            <button className="mt-4 px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-semibold rounded-lg transition-colors">Save Changes</button>
          </div>
        ))}
      </div>
    </div>
