...
import { StaffNotificationDropdown } from "@/components/staff-dashboard/StaffNotificationDropdown";
...
export function StaffTopBar({ activeRole, onRoleChange }: StaffTopBarProps) {
      ...
      <header className="sticky top-0 z-30 bg-white border-b border-slate-200/80 shadow-sm">
        <div className="flex items-center justify-between h-14 lg:h-16 px-4 lg:px-8">
          <div className="flex items-center gap-3">
            ...
            <span className={`hidden sm:inline-flex text-[10px] font-bold px-2 py-0.5 rounded-full border ${STAFF_ROLE_COLORS[activeRole]}`}>
          ...
          <div className="flex items-center gap-2">
            ...
            <Link
              ...
              className="w-8 h-8 rounded-full bg-brand-green/10 border border-brand-green/20 text-xs font-bold text-brand-green flex items-center justify-center"
            >
              {MOCK_STAFF.initials}
            </Link>
          </div>
      ...
      >
        ...
        >
          ...
          <div className="h-14 flex items-center justify-between px-4 border-b border-white/[.08] shrink-0">
            <span className="font-bold text-sm tracking-wide">Staff Portal</span>
            <button
              onClick={() => setOpen(false)}
          ...
          <div className="px-4 pt-3 pb-2">
            <select
              ...
              className="w-full text-xs bg-white/[.06] border border-white/[.12] text-white/80 rounded-lg px-2.5 py-2 focus:outline-none focus:ring-1 focus:ring-brand-green/50"
            >
